public static class ArraysTester {
    ...
    private static double[] MultiplesOf(double number, int length)
    {
        ...
        {
            // Step 3: Calculate the current multiple and store it in the array.
    ...
    private static void RotateListRight(List<int> data, int amount)
    {
        ...
        // Step 1: Normalize the rotation amount to handle cases where amount >= data.Count
        amount = amount % data.Count;

        // Step 2: If the amount is 0, the list remains unchanged
        if (amount == 0) return;

        // Step 3: Split the list into two parts
        // The part that will move to the front (last 'amount' elements)
        List<int> lastPart = data.GetRange(data.Count - amount, amount);
        // The part that will stay behind and shift right (first 'data.Count - amount' elements)
        List<int> firstPart = data.GetRange(0, data.Count - amount);

        // Step 4: Clear the original list and concatenate the two parts
        data.Clear();
